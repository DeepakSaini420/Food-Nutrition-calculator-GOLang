...
func GetNutritionalScore(n NutritionalData, st ScoreType) NutritionalScore {
	...
	if st != Water {
		...
		fiberPoints := n.Fibre.GetPoints(st)

		nagetive = n.Energy.GetPoints(st) + n.Sugar.GetPoints(st) + n.SaturatedFattyAcids.GetPoints(st) + n.Sodium.GetPoints(st)
		positive = fruitPoints + fiberPoints + n.Protien.GetPoints(st)

		if st == Cheese {
			value = nagetive - positive
		} else {
			if nagetive >= 11 && fruitPoints <= 5 {
				value = nagetive - positive - fruitPoints
			} else {
				value = nagetive - positive
			}
		}
